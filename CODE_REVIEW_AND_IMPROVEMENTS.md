...
### 2.1. Хардкодные пути и несовместимость сред (Server vs Local)
...
|---|---|---|---|
| **P0** | Конфигурация путей | Разрозненный хардкод `D:\...`, `parents[3]` | Единый модуль конфига на базе `pydantic-settings` / `.env` с автоопределением среды |
| **P0** | Логирование | Конфликт `basicConfig`, логи уходят в чужие файлы, нет ротации | Централизованный логгер с `RotatingFileHandler`, структурированным форматом |
...
## 4. Вопросы для согласования перед реализацией

1. **Окружение удаленного сервера**:
   - Какая ОС установлена на сервере (Ubuntu / Debian / Windows Server)?
   - Запускается ли сервис внутри Docker-контейнера или напрямую в виртуальном окружении (`venv`) хоста?
2. **Расположение моделей на сервере**:
   - Где на сервере хранятся веса KERYX (`/opt/models/...`, `/home/.../models` или внутри папки проекта `models/`)?
3. **Адрес LM Studio на сервере**:
   - Крутится ли LM Studio на том же сервере (`localhost:1234`) или на отдельной GPU-ноде с другим IP?
4. **Приоритетный способ интеграции с СЭД**:
   - Будет ли основным каналом оставаться **горячая папка** (`watcher.py`) с обменом через сетевой диск, или планируется перевод СЭД на вызовы по **REST API** (`api.py`)?
