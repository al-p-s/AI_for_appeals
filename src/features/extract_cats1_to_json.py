...
def parse_classifier_l1(pdf_path, output_path):
    ...
    pattern = r'(\d{4})\.0000\.0000\.0000\s+(.+?)(?=\d{4}\.\d{4}|\Z)'
    results = []
    seen = set()

    for m in re.finditer(pattern, full_text, re.DOTALL):
        full_code = m.group(1) + '.0000.0000.0000'
    ...
    print(f"Готово. L1 категорий: {len(results)}")

if __name__ == "__main__":
    parse_classifier_l1(
        r"../../data/classifier/classifier_2024.pdf",
        r"../../data/classifier/cats1.json"
    )
